  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col p-4 w-64 gap-1 bg-gray-100 rounded-md'>
        ...
        <textarea className='bg-white rounded-md border-2 border-gray-300' name="msg" rows={10} cols={30} required
          minLength={1}
          maxLength={500} />

        <label className='font-bold'>Schedule</label>
        <input className='bg-white rounded-md border-2 border-gray-300' type="datetime-local" name="targetDate" required />

        <button className=' bg-gray-500 text-white text-xl w-fit mt-4 py-2 px-4 font-bold rounded-2xl' type="submit">Send</button>
      </form>
    </>
  )
}
